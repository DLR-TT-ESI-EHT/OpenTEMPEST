...
model StackCF
  ...
  Heat.DHTVolumes2D dHT2_x0(i=Ncell, j=nY)
    annotation (Placement(transformation(extent={{-106,2},{-94,14}})));
  Heat.DHTVolumes2D dHT2_xN(i=Ncell, j=nY)
    annotation (Placement(transformation(extent={{94,0},{106,12}})));
...
equation
  ...
  // Air
  connect(airIn, manifold1.port_a);
  connect(manifold1.ports_b[1:Ncell], cell.airIn);
  connect(cell.airout, manifold_out1.ports_a[1:Ncell]);
  connect(manifold_out1.port_b, airout);
  // Fuel
  connect(fuelIn, manifold.port_a);
  connect(manifold.ports_b[1:Ncell], cell.fuelIn);
  connect(cell.fuelOut, manifold_out.ports_a[1:Ncell]);
  connect(manifold_out.port_b, fuelOut);
